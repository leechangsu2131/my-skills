...
# -*- coding: utf-8 -*-
"""
나이스(NEIS) 결석신고서 접수/결재선 상신 자동화 스크립트
========================================================
결석신고서 결재라인: 교무(강동휘) -> 교감(김경영)

사용법:
  python neis_absence_report.py --dry-run
  python neis_absence_report.py --apply --confirm APPLY_NEIS
"""
...
REMOTE_PORT = 9222
TARGET_APP_ID = "edu/sa/eds/eaa/ae/eds_eaaae03_m01" # 결석신고서 메인 앱
DETAIL_POP_ID = "edu/sa/eds/eaa/ae/eds_eaaae03_p01" # 결석신고서 상세 팝업 앱
...
def click_search(driver) -> bool:
    js = js_wrap("""
    // 결석시작일(dtAbeBgngYmd) 값을 20260501로 세팅 (조회 범위 확장)
    ...
    res = driver.execute_script(js)
    if res.get("ok"):
        print("[search] 결석시작일 20260501 변경 및 조회 클릭 성공. 2초 대기...")
        time.sleep(2.0)
        return True
    return False
...
def scan_dataset(driver) -> list:
    ...
    var key_cols = ["stuFlnm","clsNo","eduActPrcsStsNm","atrzStsNm"];
    var rows = [];
...
def process_detail_popup(driver, student_name: str) -> bool:
    # eds_eaaae03_p01 상세 팝업 (결석신고서)
    # 저장 버튼: btnUpdateSave, 닫기 버튼: btnCancel
    js_click = """
    var instances = cpr.core.Platform.INSTANCE.getAllRunningAppInstances();
    ...
    // 저장(btnUpdateSave) 버튼일 경우에만 콤보박스 세팅 트리거
    if ("TARGET_BTN" === "btnUpdateSave") {
        var cmbAbeDclrScCd = pop.lookup("cmbAbeDclrScCd");
        var cmbEduActPrcsStsCd = pop.lookup("cmbEduActPrcsStsCd");
        if (cmbAbeDclrScCd) {
            cmbAbeDclrScCd.value = "01"; // 질병결석
            cmbAbeDclrScCd.redraw();
            try {
                cmbAbeDclrScCd.dispatchEvent(new cpr.events.CValueChangeEvent("value-change", {newValue: "01"}));
            } catch(e) {}
        }
        if (cmbEduActPrcsStsCd) {
            cmbEduActPrcsStsCd.value = "02"; // 접수
            cmbEduActPrcsStsCd.redraw();
            try {
                cmbEduActPrcsStsCd.dispatchEvent(new cpr.events.CValueChangeEvent("value-change", {newValue: "02"}));
            } catch(e) {}
        }
    }
    ...
    print("  -> [저장] 클릭...")
    driver.execute_script(js_click.replace("TARGET_BTN", "btnUpdateSave"))
    time.sleep(1.5)
...
# 4) 결재선 강동휘(교무) -> 김경영(교감) 순차 추가 및 상신
...
def run(args):
    ...
    # 탭 활성화
    if not activate_tab(driver, "결석신고서관리"):
        print("[오류] '결석신고서관리' 탭을 활성화할 수 없습니다.")
        return
    ...
    for r in rows:
        print(f"  - {r.get('clsNo')}번 {r.get('stuFlnm')} | 처리상태: {r.get('eduActPrcsStsNm')} | 상신상태: {r.get('atrzStsNm')}")
...
def main():
    parser = argparse.ArgumentParser(description="결석신고서 자동 접수/상신")
