...
(function() {
    ...
    if (!main) return {error: "Main app (phe_phesm02_m00) not found"};

    var controls = [];
    var container = main.getContainer();
...
async def main():
    async with async_playwright() as p:
        try:
            print("Connecting to Chrome over CDP on port 9222 for main controls diagnosis...")
            ...
            # 상세 컨트롤 진단 구동
            result = await target_page.evaluate(JS_DIAGNOSE_MAIN_CONTROLS)
            print("MAIN CONTROLS DUMP:")
            print(json.dumps(result, ensure_ascii=True, indent=2))
            
        except Exception as e:
            print("Error occurred during main diagnosis:", e)
