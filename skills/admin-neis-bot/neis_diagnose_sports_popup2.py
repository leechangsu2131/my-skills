...
(function() {
    ...
    if (!popup2) return {error: "Popup2 app (phe_phesm02_p02) not found"};

    var controls = [];
    var container = popup2.getContainer();
...
async def main():
    async with async_playwright() as p:
        try:
            print("Connecting to Chrome over CDP on port 9222 for popup2 controls diagnosis...")
            ...
            # 상세 컨트롤 진단 구동
            result = await target_page.evaluate(JS_DIAGNOSE_POPUP2_CONTROLS)
            print("POPUP2 CONTROLS DUMP:")
            print(json.dumps(result, ensure_ascii=True, indent=2))
            
        except Exception as e:
            print("Error occurred during popup2 diagnosis:", e)
