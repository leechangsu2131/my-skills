...
async def main():
    ...
    # ROW 2 (Sn 740, 1차 연수) 회수
    print("  -> ROW 2 (1차 연수 Sn 740) 체크 및 회수 클릭...")
    js_recall_row2 = """
    (function() {
        ...
        grid.setCheckRowIndex(1, true);
        grid.selectRows([1]);
        
        var btnRtrvl = mainApp.lookup("btnRtrvl");
        if (btnRtrvl) {
            btnRtrvl.click();
            return {ok: true, sn: ds.getValue(1, "workSittnSn")};
        }
        return {error: "btnRtrvl not found"};
    })();
    """
    res = await page.evaluate(js_recall_row2)
    print("  [OK] 1차 연수 회수 클릭:", res)
