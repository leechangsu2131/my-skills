...
async def main():
    ...
    (function() {
        ...
        if (mainApp) {
            ...
            if (dmSearch) {
                try { dmSearch.setValue("workYmdFrom", "20260701"); } catch(e) {}
                try { dmSearch.setValue("workYmdTo", "20260831"); } catch(e) {}
            }
            var btnSearch = mainApp.lookup("btnSearch");
            if (btnSearch) { try { btnSearch.click(); } catch(e) {} }
        }
    })();
    """)
    ...
    print("\n================ [1차 상신 완수 후 나이스 메인 근무상황 결재상태 스캔] ================")
