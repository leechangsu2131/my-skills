...
async def main():
    ...
    (function() {
        ...
        var p00 = apps.find(function(ai) {
            return ai.app && (ai.app.id.indexOf("srv_mymmm00_p00") >= 0 || ai.app.id.indexOf("srv_mymmm00_p") >= 0);
        });
        if (!p00) return {error: "p00 not found"};
        var ctrls = [];
        p00.getContainer().getAllRecursiveChildren().forEach(function(c) {
            ctrls.push({
                id: c.id || "",
                type: c.type || "",
        ...
        return {appId: p00.app.id, ctrls: ctrls};
    })();
    """
    res = await page.evaluate(js_inspect_p00)
    print("\n================ P00 CONTROLS ================")
    print("APP ID:", res.get("appId"))
    for c in res.get("ctrls", []):
        if c["type"] == "button" or "btn" in c["id"].lower() or "결재" in c["text"] or "승인" in c["text"] or "상신" in c["text"] or "저장" in c["text"]:
