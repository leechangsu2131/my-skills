...
- M6: 채점 확정 결과를 학생 시험지 위에 마킹하고 PDF로 내보내는 단계
- M7: CSV/XLSX로 결과를 내보내는 단계
...
- [x] `/grading` 채점 결과 목록과 학생별 교사 검토 UI
- [x] 대시보드 6번 `마킹본 만들기`
- [x] 학생별 마킹 PDF, 전체 통합 PDF, 학생별 PDF ZIP 산출

## 2) Guiding Principles

- [x] Gemini 웹 사용 흐름은 유지하고, 하류(ingest -> OCR -> 채점 -> 검토 -> 마킹/내보내기)만 확장
- [x] `/review`는 정렬/영역 검수 전용으로 유지, 채점 UI는 `/grading`으로 분리
- [x] 모든 단계 결과를 프로젝트 아티팩트(JSON/이미지/PDF)로 저장해 재실행 가능하게 설계
- [x] 기존 YOLO 산출물 경로는 병행 유지
...
### M5. 교사용 검토 UI
- [x] `webapp/templates/grading_overview.html` 추가
- [x] `webapp/templates/grading_student.html` 추가
- [x] `GET/POST /grading/student/{student_id}`로 수동 정정 반영
- [x] `GET /grading/gemini-review` 저신뢰 문항 재판독 보조 화면 추가
- [x] 테스트: `tests/test_grading_student_page.py` 확장
- [x] 완료 기준: 저신뢰 문항 교정 후 재저장/재집계 가능

### M6. 마킹본 생성 및 PDF 다운로드
- [x] `src/marked_exam_renderer.py` 추가
- [x] `ProjectPaths.marked_dir` 추가
- [x] `POST /api/grading/mark/{student_id}` 학생별 마킹본 생성
- [x] `POST /api/grading/mark-all` 전체 마킹본 생성
- [x] `GET /api/grading/marked/{student_id}/pdf` 학생별 PDF 미리보기
- [x] `GET /api/grading/marked/{student_id}/download` 학생별 PDF 다운로드
- [x] `GET /api/grading/marked/all/pdf` 전체 통합 PDF 미리보기/다운로드
- [x] `GET /api/grading/marked/all/download` 학생별 PDF ZIP 다운로드
- [x] 대시보드 6번 `마킹본 만들기` 추가
- [x] 완료 기준: 첫 페이지 상단 중앙 점수 표시, 답안 영역 기준 빨간 연필 스타일 O/슬래시 마킹, 학생별/전체 PDF 산출 가능
...
- [x] `<project>/artifacts/marked/students/<student_id>/`
- [x] `<project>/artifacts/marked/marked_exams_combined.pdf`
- [x] `<project>/artifacts/marked/marked_exams_all.zip`
- [ ] `<project>/artifacts/exports/`

## 5) Test Gate (각 마일스톤 공통)

- [ ] 신규 단위 테스트 우선 작성 -> 실패 확인 -> 구현 -> 통과 확인
- [x] 기존 회귀: `.venv\Scripts\python.exe -m pytest -q` 통과
- [ ] 핵심 라우트 수동 점검: `/dashboard`, `/review`, `/grading`

## 6) Documentation Gate

- [x] 각 마일스톤 완료 시 `README.md` 워크플로우 업데이트
...
- [x] 결과: `29 passed`
- [x] 8080 `/dashboard` 응답에서 `마킹본 만들기`, `전체 통합 PDF`, `학생별 PDF ZIP` 확인

## 8) Next Action

- [ ] M7 Step 1: CSV/XLSX 성적표 export 설계 및 테스트 추가
- [ ] M7 Step 2: 학생별 총점/문항별 정오율 summary row 생성
- [ ] M7 Step 3: `/api/export/results`, `/api/export/{name}` 추가
