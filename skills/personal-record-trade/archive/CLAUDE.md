...
### 6. 매도 및 청산 기록 원칙 (오류 방지)
- **매도 시 포트폴리오 업데이트**: 매매일지에 매도 기록을 추가한 후에는, 반드시 `📊 포트폴리오` 탭에서 해당 종목의 보유 수량을 감소시키고, 매도 대금만큼 '원화' (또는 '달러', '헤알' 등 해당 통화 현금) 자산에 금액을 합산해야 합니다.
- **전량 매도(청산) 순서 주의**: 특정 종목을 전량 매도할 때는 **반드시 수량을 0으로 차감하기 전에** 해당 종목의 평가손익과 수익률 등 최종 데이터를 `👋 청산종목` 탭으로 먼저 복사(기록)해야 합니다. (수량을 먼저 0으로 바꾸면 시트 내 수식이 0으로 재계산되어 실제 수익률 데이터가 날아가 버리는 race condition이 발생합니다.) 이후 포트폴리오 탭에서 해당 종목 행을 삭제합니다.
