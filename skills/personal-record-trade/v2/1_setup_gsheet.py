...
실행 전: pip install gspread google-auth

인증 방법:
  - service_account.json (서버/자동화용, 권장)
...
def banded_rows_req(sheet_id, start_row, end_row, num_cols,
    ...
    return {
        "addBanding": {
            "bandedRange": {
                "bandedRangeId": sheet_id * 10 + start_row,
