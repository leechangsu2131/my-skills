...
def get_client():
    cred = os.path.join(BASE_DIR, "service_account.json")
    if os.path.exists(cred):
        return gspread.authorize(Credentials.from_service_account_file(cred, scopes=SCOPES))
    return gspread.oauth()

def get_sheet_id():
    f = os.path.join(BASE_DIR, "sheet_id.txt")
    if os.path.exists(f):
        return open(f).read().strip()
    raise FileNotFoundError("sheet_id.txt 없음. 먼저 1_setup_gsheet.py를 실행하세요.")
...
def build_correlation_sheet(ss, price_data_start_row):
    ...
    price_cols = "ABCDEFGHIJK"  # B=1번째종목, C=2번째, ...
