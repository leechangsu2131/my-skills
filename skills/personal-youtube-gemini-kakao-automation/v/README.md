...
- PC가 꺼져 있거나 **절전·최대 절전** 상태
- **로그아웃** 상태 (Windows에 로그인 안 됨)
- 작업을 **사용 안 함**으로 꺼 둔 경우
- 주말 (스케줄이 월~금만)

### 화면보호기는 괜찮나요?

**화면보호기만** 켜져 있고 Windows에 **로그인은 유지**된 상태면, 대체로 **실행됩니다**.

다만 **Win+L 잠금**·**절전**·**노트북 덮개 닫힘(절전 설정)** 은 실패할 수 있습니다.  
`RUN_TIME` 전후에는 PC가 깨어 있고 로그인만 되어 있으면 됩니다.

자세한 표·권장 설정: [troubleshooting.md — 화면보호기 / 잠금 / 절전](./troubleshooting.md#화면보호기--잠금--절전-2026-06-01-정리)
