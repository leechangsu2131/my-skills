...
def search_place(query: str) -> tuple:
    """키워드 검색 → (lng, lat, 주소, 장소명)"""
    url = "https://dapi.kakao.com/v2/local/search/keyword.json"
    for code in ["SC4", ""]:  # 학교 카테고리 우선, 폴백 일반
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if docs:
                    ...
                    best = max(docs, key=lambda d: sum(c in d["place_name"] for c in q_clean))
                    return (
...
def get_route(o_lng, o_lat, d_lng, d_lat, priority="RECOMMEND") -> tuple:
    ...
    try:
        ...
        if res.status_code in (401, 403):
            return None, None, f"길찾기 API 인증 오류 ({res.status_code}) - 카카오 개발자 센터에서 카카오내비 활성화 필요"
        if res.status_code != 200:
...
with tab1:
    ...
    if xl:
        ...
        df1 = pd.read_excel(file_src, sheet_name=sheet1, header=int(hrow1)-1)
        ...
        if st.button("🚀 계산 시작", type="primary", use_container_width=True, key="t1_btn",
            ...
            with st.spinner("출발 위치 조회 중..."):
                my_lng, my_lat, my_addr, _ = search_place(my_loc1)
...
with tab2:
    ...
    if st.button("🚀 매트릭스 계산", type="primary", use_container_width=True, key="t2_btn",
        ...
        if HAS_FOLIUM:
            ...
            st_folium(m2, use_container_width=True, height=450)
