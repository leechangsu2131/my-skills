...
POST_PRINT_TRAY=15   # 인쇄 종료 후 되돌릴 기본 용지함 (후지필름 기준 15=자동, 1=트레이1)
```
...
> **일반 문서 트레이 복구**: 배치 인쇄 뒤 일반 문서가 계속 트레이2만 찾으면 `reset_printer_tray.bat`을 실행해 사용자별 기본 용지함을 `POST_PRINT_TRAY` 값으로 되돌리세요.
