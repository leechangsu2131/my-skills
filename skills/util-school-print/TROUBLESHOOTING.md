...
## 5. 일반 문서가 계속 트레이2만 찾는 경우

### 🚨 문제 상황
- 배치 인쇄 이후 다른 A4 문서를 인쇄하려고 하면, 트레이1에 A4가 있어도 트레이2에 A4가 없다는 이유로 멈춤.
- Windows 프린터 설정 화면에서 용지함을 돌린 것 같아도 계속 트레이2만 사용하려고 함.

### 💡 원인
- 이 스크립트는 `PRINTER_INFO_9`의 사용자별 기본값(`DEVMODE.DefaultSource`)을 바꿔서 트레이를 전환합니다.
- Windows UI에서 보이는 기본값과 별개로, 현재 로그인 사용자에게만 적용되는 Level 9 값이 남을 수 있습니다.
- 후지필름 `FUJIFILM Apeos C2561 3연구실` 기준 확인된 값:
  - `15` = 자동
  - `1` = 트레이 1
  - `3` = 트레이 2

### ✅ 복구 방법
1. `reset_printer_tray.bat`을 더블클릭합니다.
2. 출력이 `사용자 기본 용지함(Level 9): 3 -> 15`처럼 나오면 복구된 것입니다.
3. 이후 일반 문서는 자동 용지함(`15`) 기준으로 인쇄됩니다.

### 재발 방지
- `.env`의 `POST_PRINT_TRAY=15` 설정 때문에 `batch_print_v2.py`는 배치 인쇄 종료 시 사용자별 기본 용지함을 자동으로 `15`로 정리합니다.
- 자동 대신 항상 트레이1로 돌리고 싶으면 `POST_PRINT_TRAY=1`로 바꾸면 됩니다.

### 그래도 한글의 다른 문서가 계속 트레이2를 찾는 경우
- `ParameterSetObject.pdf` 기준 `HPrint`에는 `PrinterPaperSize`, `PrinterPaperWidth`, `PrinterPaperLength`, `PrinterName`은 있지만 트레이/용지함(`DEVMODE.dmDefaultSource`) 파라미터는 없습니다.
- 따라서 Windows 프린터 기본값이 정상인데도 한글에서만 트레이2를 찾으면, 한글이 레지스트리에 저장한 마지막 인쇄 파라미터가 남아 있을 가능성이 큽니다.
- 한글 문서를 모두 저장하고 한글을 완전히 종료한 뒤 `reset_hwp_print_settings.bat`을 실행하세요.
- 이 도구는 `HKCU\Software\HNC\Hwp\*\Parameters\00000215` 값을 JSON으로 백업한 뒤 삭제하고, 프린터 기본 용지함도 `POST_PRINT_TRAY` 값으로 되돌립니다.
